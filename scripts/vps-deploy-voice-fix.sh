...
echo "  Logs MUST show: tts_text_present=true bangla_script=true azure=bn-BD-PradeepNeural skip_tts=false"
echo "  audio_url_sent may be true (Chirp3 best-effort) or false (Pradeep only) — never omit Bangla text."
echo "  Optional:"
echo "  docker logs maskara-worker 2>&1 | grep -E 'audio_url_sent|tts_text_present|Pradeep|refuse audio_url'"
echo ""
...
echo "  5) If still English female: ePBX portal language/voice/IVR — disable default English IVR"
